...
/**
 * js/admin/login.js — Login për menagjerët
 */
...
window.AdminLogin = (() => {
    ...
    async function login(email, password) {
        ...
        try {
            ...
            // Kontrollo rolin
            const allowedRoles = ['supervisor', 'manager', 'director', 'admin'];
            if (!allowedRoles.includes(operator.role)) {
                await firebase.auth().signOut();
                showError('Roli juaj nuk lejon qasje në panel.');
                return null;
            }
            ...
            // Ruaj në storage
            window.TaxiStorage?.set('taxi.admin', {
                id: operator.id,
                name: operator.name,
            ...
            console.log('✅ Menagjeri u ngarkua:', operator.name, '| Roli:', operator.role);
            return operator;
    ...
    async function checkExistingSession() {
        const saved = window.TaxiStorage?.get('taxi.admin');
    ...
    function init() {
        document.getElementById('btn-login')?.addEventListener('click', async () => {
            ...
            if (operator && window.AdminApp) {
                window.AdminApp.onLoginSuccess(operator);
            }
        });
        ...
        console.log('✅ AdminLogin init');
    }

    return { init, login, checkExistingSession, getOperatorByEmail };
})();

console.log('✅ admin/login.js ngarkuar');
