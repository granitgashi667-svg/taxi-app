...
/**
 * operators.js — Menaxhimi i operatorëve + statistikat
 */
...
window.TaxiOperators = (() => {
    ...
    async function getOrCreate(user) {
        ...
        try {
            ...
            if (doc.exists) {
                ...
                startHeartbeat();
                console.log('✅ Operator u ngarkua:', doc.data().name);
                ...
                sessionStart = Date.now();
                startHeartbeat();
                console.log('✅ Operator i re u krijua:', newOp.name);
                return { id: user.uid, ...newOp };
            }
        } catch (e) {
    ...
    function startHeartbeat() {
        ...
        console.log('💓 Heartbeat aktivizuar');
    }
    ...
    async function incrementStats(uid, increments) {
        ...
        } catch (e) { console.error('❌ incrementStats:', e); }
    }
    ...
    function setupTracking() {
        ...
        console.log('✅ Tracking aktivizuar');
    }
    ...
    async function stopSession() {
        ...
        if (currentOperatorId && sessionStart) {
            ...
            if (minutes > 0) await incrementStats(currentOperatorId, { totalMinutes: minutes });
        }
    ...
    return { getOrCreate, incrementStats, getStats, setupTracking, stopSession };
})();
